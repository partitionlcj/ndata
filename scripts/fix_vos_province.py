...
with db.cursor() as c:
    c.execute("select request_id from sariel.debug_query where province = '' or province is null")
    rs = c.fetchall()
    i = 0
    for r in rs:
        ...
        c.execute('update sariel.debug_query set city=%s, province=%s where request_id=%s',(city,province,rid))
        i = i + 1
        if i % 1000 == 0:
            db.commit()
