...
def pipeline():
    ...
    with db_ri.cursor() as c1, db.cursor() as c2:
        ...
        print("select * from request_info where query is not null and env='ds-gn-prod' and `timestamp`>=%s and `timestamp`<%s",(ts,ts_end))
        c1.execute("select * from request_info where query is not null and env='ds-gn-prod' and `timestamp`>=%s and `timestamp`<%s",(ts,ts_end))
        rs1 = c1.fetchall()
