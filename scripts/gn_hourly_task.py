...
def gen_daily_data(c,dt,today):
  ...
  ssdb_save_json(f"ds-mars-prod.total.info.use_nomi.d.{today.strftime('%Y%m%d')}", r)
...
def gen_hourly_data(c):
  ...
  print(json.dumps(r))
  ssdb_save_json(f"ds-mars-prod.total.info.use_nomi_hours.d.{today.strftime('%Y%m%d')}", r)
